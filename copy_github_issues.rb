...
distination_repository = "distination_organization/distination_repository"
...
issues.each.with_index(1) do |issue, index|
  ...
  client.create_issue(distination_repository, issue.title, issue.body)
  comments = client.issue_comments source_repository, issue.number, options
  comments.each.with_index(1) do |comment, index|
    p "#{comment.body}"
    client.add_comment(distination_repository, issue.number, comment.body)
    sleep(3)
  end
end
